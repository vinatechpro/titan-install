...
translate() {
  ...
  case "$LANGUAGE" in
    ...
    "id")
      case "$text" in
        "Script ini harus dijalankan dengan hak akses root.") echo "Skrip ini harus dijalankan dengan hak akses root.";;
        "Perintah '$1' gagal.") echo "Perintah '$1' gagal.";;
        "Memeriksa Snap...") echo "Memeriksa Snap...";;
        "Snap sudah terpasang.") echo "Snap sudah terpasang.";;
        "Snap belum terpasang, mulai pemasangan...") echo "Snap belum terpasang, mulai pemasangan...";;
        "Sistem adalah Debian/Ubuntu.") echo "Sistem adalah Debian/Ubuntu.";;
        "Sistem adalah Fedora.") echo "Sistem adalah Fedora.";;
        "Sistem adalah CentOS/RHEL.") echo "Sistem adalah CentOS/RHEL.";;
        "Sistem tidak didukung. Tidak dapat memasang Snap secara otomatis.") echo "Sistem tidak didukung. Tidak dapat memasang Snap secara otomatis.";;
        "Snap berhasil terpasang.") echo "Snap berhasil terpasang.";;
        "Memasang Multipass...") echo "Memasang Multipass...";;
        "Multipass berhasil terpasang.") echo "Multipass berhasil terpasang.";;
        "Memeriksa Multipass...") echo "Memeriksa Multipass...";;
        "Multipass siap digunakan.") echo "Multipass siap digunakan.";;
        "Silakan masukkan key Titan Agent: ") echo "Silakan masukkan key Titan Agent: ";;
        "Anda belum memasukkan key. Skrip berhenti.") echo "Anda belum memasukkan key. Skrip berhenti.";;
        "Key tidak valid. Skrip berhenti.") echo "Key tidak valid. Skrip berhenti.";;
        "Mengunduh dan mengekstrak Titan Agent...") echo "Mengunduh dan mengekstrak Titan Agent...";;
        "Titan Agent berhasil diunduh dan diekstrak.") echo "Titan Agent berhasil diunduh dan diekstrak.";;
        "Membuat file service systemd...") echo "Membuat file service systemd...";;
        "Mengaktifkan service systemd...") echo "Mengaktifkan service systemd...";;
        "Memulai service systemd...") echo "Memulai service systemd...";;
        "Memeriksa status service systemd...") echo "Memeriksa status service systemd...";;
       "Titan Agent telah terpasang dan berjalan sebagai service systemd.") echo "Titan Agent telah terpasang dan berjalan sebagai service systemd.";;
        "Proses instalasi dan menjalankan Titan Agent selesai.") echo "Proses instalasi dan menjalankan Titan Agent selesai.";;
         *) echo "$text";;
      esac
      ;;
    "ru")
      case "$text" in
        "Script ini harus dijalankan dengan hak akses root.") echo "Этот скрипт должен быть запущен с правами root.";;
        "Perintah '$1' gagal.") echo "Команда '$1' не удалась.";;
       "Проверка Snap...") echo "Проверка Snap...";;
        "Snap уже установлен.") echo "Snap уже установлен.";;
